...
describe("deploy contract", function () {
    it('deploy contract', async function () { 
        ERC20Instance = await TestContract.new(...ERC20Detail, { from: owner });
    });
});
...
describe("first transfers to accounts", function () {
    ...
    it('reverts when transferring tokens to the zero address', async function () {
        ...
        await expectRevert(
          ERC20Instance.transfer(constants.ZERO_ADDRESS, ether(tEther), { from: owner }),
...
describe("make first snapshot", async function () { 
    ...
    it('reverts when non-owner address make snapshot', async function () {
        // Conditions that trigger a require statement can be precisely tested
        await expectRevert(
          ERC20Instance.snapshot({ from: sender }),
...
describe("check first snapshot", async function () {
    ...
    it('check first snapshot for totalSupply: totalSupplyAt()', async function () {
        assert.equal(ether(ERC20Detail[2].toString()).toString(), (await ERC20Instance.totalSupplyAt(snapshotId)).toString());
    });
});
